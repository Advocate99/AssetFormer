...
class CaptionEmbedder(nn.Module):
    ...
    """
    def __init__(self, in_channels, hidden_size, uncond_prob, token_num=24):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.cls_embedding = CaptionEmbedder(config.caption_dim, config.dim, config.class_dropout_prob, self.cls_token_num)
...
def precompute_freqs_cis(seq_len: int, n_elem: int, base: int = 10000, cls_token_num=24):
...
def precompute_freqs_cis_2d(grid_size: int, n_elem: int, base: int = 10000, cls_token_num=24):
